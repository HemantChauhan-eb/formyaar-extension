...
function showContextualBanner() {
  ...
  <span style="font-weight:200;color:rgba(255,255,255,0.7);">Form</span><span style="color:#E8930A;font-weight:800;">·</span><span style="font-weight:800;color:#ffffff;">Yaar</span>
      ...
      <div style="flex:1;overflow-y:auto;padding:18px 16px 24px;position:relative;">
        ...
        <div style="position:relative;z-index:1;">
          <!-- Amount hero -->
          <div style="background:linear-gradient(135deg,#821cff 0%,#5a0bb5 100%);border-radius:14px;padding:18px 20px;color:#fff;margin-bottom:18px;position:relative;overflow:hidden;">
            <div style="position:absolute;right:-14px;top:-14px;pointer-events:none;opacity:0.1;">
              <svg width="90" height="90" viewBox="0 0 90 90"><circle cx="45" cy="45" r="41" fill="none" stroke="white" stroke-width="5.4"/><circle cx="45" cy="45" r="10.8" fill="white"/>${Array.from(
                { length: 24 },
                (_, i) => {
                  const a = (i * 15 * Math.PI) / 180;
                  return `<line x1="${45 + 11.25 * Math.cos(a)}" y1="${45 + 11.25 * Math.sin(a)}" x2="${45 + 33.75 * Math.cos(a)}" y2="${45 + 33.75 * Math.sin(a)}" stroke="white" stroke-width="3.15"/>`;
                },
              ).join("")}</svg>
            </div>
            <div style="font-size:11px;opacity:0.8;font-weight:500;letter-spacing:0.3px;">PAN CARD — NEW APPLICATION</div>
            <div style="font-size:32px;font-weight:800;margin-top:4px;letter-spacing:-0.5px;">₹107</div>
            <div style="font-size:10.5px;opacity:0.72;margin-top:4px;">Govt. fee ₹93 + Service fee ₹14 (incl. GST)</div>
          ...
          <div style="display:flex;gap:7px;margin-bottom:16px;">
            <button id="fy-tab-upi" class="fy-method-btn" onclick="fySetMethod('upi')" style="flex:1;padding:8px 4px;border-radius:9px;cursor:pointer;border:1.5px solid #821cff;background:rgba(130,28,255,0.055);font-size:11px;font-weight:700;color:#821cff;letter-spacing:0.3px;">UPI</button>
            <button id="fy-tab-card" class="fy-method-btn" onclick="fySetMethod('card')" style="flex:1;padding:8px 4px;border-radius:9px;cursor:pointer;border:1.5px solid #ddd;background:#fff;font-size:11px;font-weight:700;color:#888;letter-spacing:0.3px;">Card</button>
            <button id="fy-tab-netbank" class="fy-method-btn" onclick="fySetMethod('netbank')" style="flex:1;padding:8px 4px;border-radius:9px;cursor:pointer;border:1.5px solid #ddd;background:#fff;font-size:11px;font-weight:700;color:#888;letter-spacing:0.3px;">Net Banking</button>
          </div>

          <!-- UPI form -->
          <div id="fy-form-upi" style="display:flex;flex-direction:column;gap:13px;animation:fy-fadeIn 0.15s ease;">
            <div>
              <label style="font-size:11.5px;font-weight:600;color:#50507a;display:block;margin-bottom:6px;letter-spacing:0.2px;">UPI ID</label>
              <input placeholder="yourname@okaxis" style="width:100%;padding:11px 14px;border:1.5px solid #e0e0ee;border-radius:10px;font-size:13.5px;outline:none;color:#0a0a2e;background:#fff;"/>
            </div>
            <div style="display:flex;align-items:center;gap:10px;">
              <div style="flex:1;height:1px;background:#e8e8ee;"></div>
              <span style="font-size:11px;color:#bbb;font-weight:500;">or choose app</span>
              <div style="flex:1;height:1px;background:#e8e8ee;"></div>
            </div>
            <div style="display:flex;gap:8px;">
              <button style="flex:1;padding:9px 4px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:12px;font-weight:700;color:#555;cursor:pointer;">GPay</button>
              <button style="flex:1;padding:9px 4px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:12px;font-weight:700;color:#555;cursor:pointer;">PhonePe</button>
              <button style="flex:1;padding:9px 4px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:12px;font-weight:700;color:#555;cursor:pointer;">Paytm</button>
            </div>
          </div>

          <!-- Card form -->
          <div id="fy-form-card" style="display:none;flex-direction:column;gap:12px;animation:fy-fadeIn 0.15s ease;">
            <div>
              <label style="font-size:11.5px;font-weight:600;color:#50507a;display:block;margin-bottom:6px;">Card Number</label>
              <input placeholder="1234 5678 9012 3456" style="width:100%;padding:11px 14px;border:1.5px solid #e0e0ee;border-radius:10px;font-size:13.5px;outline:none;color:#0a0a2e;background:#fff;"/>
            </div>
            <div>
              <label style="font-size:11.5px;font-weight:600;color:#50507a;display:block;margin-bottom:6px;">Name on Card</label>
              <input placeholder="RAHUL SHARMA" style="width:100%;padding:11px 14px;border:1.5px solid #e0e0ee;border-radius:10px;font-size:13.5px;outline:none;color:#0a0a2e;background:#fff;text-transform:uppercase;"/>
            </div>
            <div style="display:flex;gap:10px;">
              <div style="flex:1;">
                <label style="font-size:11.5px;font-weight:600;color:#50507a;display:block;margin-bottom:6px;">Expiry</label>
                <input placeholder="MM / YY" style="width:100%;padding:11px 14px;border:1.5px solid #e0e0ee;border-radius:10px;font-size:13.5px;outline:none;color:#0a0a2e;background:#fff;"/>
              </div>
              <div style="flex:1;">
                <label style="font-size:11.5px;font-weight:600;color:#50507a;display:block;margin-bottom:6px;">CVV</label>
                <input placeholder="•••" type="password" style="width:100%;padding:11px 14px;border:1.5px solid #e0e0ee;border-radius:10px;font-size:13.5px;outline:none;color:#0a0a2e;background:#fff;"/>
              </div>
            </div>
          </div>

          <!-- Net banking -->
          <div id="fy-form-netbank" style="display:none;animation:fy-fadeIn 0.15s ease;">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
              <button style="padding:11px 8px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:13px;font-weight:700;color:#444;cursor:pointer;">SBI</button>
              <button style="padding:11px 8px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:13px;font-weight:700;color:#444;cursor:pointer;">HDFC</button>
              <button style="padding:11px 8px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:13px;font-weight:700;color:#444;cursor:pointer;">ICICI</button>
              <button style="padding:11px 8px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:13px;font-weight:700;color:#444;cursor:pointer;">Axis</button>
              <button style="padding:11px 8px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:13px;font-weight:700;color:#444;cursor:pointer;">Kotak</button>
              <button style="padding:11px 8px;border:1.5px solid #e0e0ee;border-radius:9px;background:#fff;font-size:13px;font-weight:700;color:#444;cursor:pointer;">PNB</button>
            </div>
            <div style="margin-top:10px;">
              <label style="font-size:11.5px;font-weight:600;color:#50507a;display:block;margin-bottom:6px;">Other Bank</label>
              <input placeholder="Search your bank..." style="width:100%;padding:11px 14px;border:1.5px solid #e0e0ee;border-radius:10px;font-size:13.5px;outline:none;color:#0a0a2e;background:#fff;"/>
            </div>
          </div>

          <!-- Pay button -->
          <button id="fy-pay-btn" class="fy-pay-btn" style="margin-top:20px;width:100%;padding:14px;background:#821cff;color:#fff;border:none;border-radius:12px;font-weight:800;font-size:15px;cursor:pointer;box-shadow:0 5px 20px rgba(130,28,255,0.27);transition:all 0.2s ease;letter-spacing:0.3px;display:flex;align-items:center;justify-content:center;gap:8px;">
  ...
  // Method tab switcher (global function needed for inline onclick)
  (window as any).fySetMethod = (method: string) => {
    ["upi", "card", "netbank"].forEach((m) => {
      const tab = document.getElementById(`fy-tab-${m}`);
      const form = document.getElementById(`fy-form-${m}`);
      if (!tab || !form) return;
      const active = m === method;
      tab.style.border = active ? "1.5px solid #821cff" : "1.5px solid #ddd";
      tab.style.background = active ? "rgba(130,28,255,0.055)" : "#fff";
      tab.style.color = active ? "#821cff" : "#888";
